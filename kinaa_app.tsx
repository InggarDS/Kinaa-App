...
import { getFirestore, doc, setDoc, getDoc, collection, onSnapshot, addDoc, deleteDoc, query } from 'firebase/firestore';
import { 
  Baby, Timer, Droplet, BarChart2, Play, Pause, Square, Save, 
  Settings, CheckCircle2, Mail, Lock, LogOut, Loader2
} from 'lucide-react';

// --- Konfigurasi Firebase (Standar Canvas) ---
...
export default function App() {
  ...
  // State UI Aplikasi
  const [isAppAuthenticated, setIsAppAuthenticated] = useState(() => sessionStorage.getItem('kinaa_auth') === 'true');
...
function AuthScreen({ firebaseUser, appProfile, onAuthSuccess, showToast }) {
  // Jika profil belum ada di database, arahkan ke mode Register
    ...
    try {
      if (mode === 'register') {
        ...
        await setDoc(profileRef, {
          ...
          password, // Pada MVP nyata disimpan terenkripsi, ini untuk simulasi MVP
          name: babyName,
          dob: babyDob,
          createdAt: Date.now()
        });
        ...
        // Validasi Login terhadap Profil di Firestore
...
function ChartView({ profile, logs }) {
  const getTargetVolume = () => {
    ...
    const birthDate = new Date(profile.dob);
    const today = new Date();
    const ageInMonths = Math.abs(today - birthDate) / (1000 * 60 * 60 * 24 * 30);
